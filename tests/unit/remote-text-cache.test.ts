...
describe("fetchTextCached", () => {
  test("writes successful fetches and reuses fresh cache", async () => {
    ...
    let calls = 0;
    const first = await fetchTextCached("https://example.com/a", { cacheDir, fetcher: async () => { calls += 1; return "fresh"; } });
    const second = await fetchTextCached("https://example.com/a", { cacheDir, fetcher: async () => { calls += 1; return "new"; } });
  ...
  test("returns stale cache when refresh fails", async () => {
    ...
    const stale = await fetchTextCached("https://example.com/b", { cacheDir, ttlMs: -1, fetcher: async () => { throw new Error("offline"); } });
    expect(stale).toBe("cached");
  });

  test("throws refresh failures when stale fallback is disabled", async () => {
    const cacheDir = tempCache();
    await expect(fetchTextCached("https://example.com/c", { cacheDir, staleOnError: false, fetcher: async () => { throw new Error("offline"); } })).rejects.toThrow("offline");
  });

  test("throws when refresh fails and no stale cache exists", async () => {
    await expect(fetchTextCached("https://example.com/miss", { cacheDir: tempCache(), fetcher: async () => { throw new Error("offline"); } })).rejects.toThrow("offline");
  });
  ...
  test("default fetcher rejects non-OK responses", async () => {
    ...
    try {
      await expect(fetchTextCached("https://example.com/status", { cacheDir: tempCache(), staleOnError: false })).rejects.toThrow("500");
    } finally {
