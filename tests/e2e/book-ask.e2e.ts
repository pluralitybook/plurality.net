...
test.describe('book-ask client', () => {
  ...
  test('search overlay has tappable magnifier submit, no clear button', async ({ page }) => {
    await page.goto('/');
  ...
  test('ask answer renders footnotes as fully-line clickable numbered sources', async ({
    page,
    baseURL,
  }) => {
    ...
    const apiReady = await page.evaluate(() => {
      ...
      if (
        !('PluralityBookAsk' in w) ||
        typeof w.PluralityBookAsk !== 'object' ||
        w.PluralityBookAsk === null
      )
        return false;
    ...
    await page.evaluate(() => {
      const w: unknown = window;
      if (!(typeof w === 'object' && w !== null && 'PluralityBookAsk' in w))
        return Promise.resolve();
      const api: unknown = w.PluralityBookAsk;
      if (
        typeof api !== 'object' ||
        api === null ||
        !('runAsk' in api) ||
        typeof api.runAsk !== 'function'
      ) {
        return Promise.resolve();
      }
