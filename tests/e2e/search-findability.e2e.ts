...
test.describe('search findability', () => {
  ...
  test('latin query falls back to English edition', async ({ page }) => {
    ...
    await expect(page.locator('.plurality-search__en-note')).toContainText('英語版での完全一致');

    const enResult = page.locator('.plurality-search__en-note ~ .pagefind-ui__result');
...
test.describe('ask history', () => {
  test('chip restores answer without refetch', async ({ page }) => {
    ...
    await expect(page.locator('#plurality-ask-answer .plurality-ask-answer__body')).toContainText(
      'Plurality',
      { timeout: 10_000 }
    );
    ...
    await expect(page.locator('#plurality-ask-answer .plurality-ask-answer__cursor')).toHaveCount(
      0,
      { timeout: 5_000 }
    );
