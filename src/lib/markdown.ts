...
export function splitByHeadings(raw: string): SearchSubsection[] {
  ...
  if (strippedTrailing) subsections.push({ heading: lastHeading, anchor: lastAnchor, content: strippedTrailing });
  return subsections;
}
...
export function splitByBlockquotes(raw: string): SearchSubsection[] {
  ...
  for (const part of parts) {
    ...
    subsections.push({ heading: null, anchor: `endorsement-${subsections.length + 1}`, content: stripped });
  }
  return subsections;
}
