...
export function sectionLabel(langI18n: LangI18n | undefined, sectionTitle: string, number: string): string {
  const name = langI18n?.sections?.[sectionTitle] || sectionTitle;
  return `${name} · ${number}`;
}

export function buildCreditsChapterEntry(lang: string, langData: LangData | undefined, credits: Credits, chapters: Chapters): SearchChapter | null {
...
export async function buildChapterEntry({ lang, langData, langI18n, section, chapter, fetcher }: BuildChapterEntryArgs): Promise<SearchChapter | null> {
...
export async function buildLangEntries({ lang, langData, langI18n, chapters, fetcher }: BuildLangEntriesArgs): Promise<SearchChapter[]> {
  if (!langData?.files) return [];
  const entries: SearchChapter[] = [];
  for (const section of chapters.sections) {
    for (const chapter of section.chapters || []) {
      const entry = await buildChapterEntry({ lang, langData, langI18n, section, chapter, fetcher });
...
export async function buildSearchIndex({ targetLangs, translations, i18n, chapters, credits, fetcher }: BuildSearchIndexArgs): Promise<Record<string, SearchChapter[]>> {
  const result: Record<string, SearchChapter[]> = {};
  for (const lang of targetLangs) {
    const entries = await buildLangEntries({ lang, langData: translations[lang], langI18n: i18n[lang] || {}, chapters, fetcher });
